...
function buildActualFromActivity(act) {
  ...
  const paceStr = cat === "run" ? speedToPace(act.average_speed) : null;
  return {
    distance: distKm,
    duration: timeStr,
    pace: paceStr || "",
...
// Returns { matched: updatedPlan, unassigned: [activities] }
function matchActivitiesToPlan(activities, plan) {
  ...
  activities.forEach(act => {
    ...
    // Already assigned from a previous sync
    const alreadyIn = updated.find(d => d.actual?.stravaId === act.id);
    if (alreadyIn) { usedActivityIds.add(act.id); return; }

    // Try exact date match
    const dayIdx = updated.findIndex(d => d.date === actDate && planDayAcceptsCategory(d.session, cat) && !d.actual?.stravaId);
    if (dayIdx !== -1) {
      updated[dayIdx] = { ...updated[dayIdx], completed: true, actual: buildActualFromActivity(act) };
      usedActivityIds.add(act.id);
    } else {
      // No match → goes to unassigned tray
...
function generatePlan() {
  ...
  const days = [];
  const pace = {
    easy: "5:40–6:10 /km", marathon: "4:16 /km", recovery: "6:00–6:30 /km",
  };
  const gym = {
    ...
    specific: { label: "Fuerza Específica Corredor", exercises: [{ name: "Sentadilla búlgara", sets: 3, reps: 8, kg: 20 }, { name: "Press banca", sets: 3, reps: 8, kg: 52 }, { name: "Peso muerto", sets: 3, reps: 6, kg: 60 }, { name: "Hip thrust", sets: 3, reps: 12, kg: 50 }, { name: "Core: dead bug", sets: 3, reps: 10, kg: 0 }, { name: "Gemelos en máquina", sets: 4, reps: 15, kg: 30 }] },
    taper: { label: "Fuerza Mantenimiento (Taper)", exercises: [{ name: "Sentadilla", sets: 2, reps: 8, kg: 45 }, { name: "Press banca", sets: 2, reps: 8, kg: 45 }, { name: "Hip thrust", sets: 2, reps: 10, kg: 40 }, { name: "Plancha", sets: 2, reps: 1, duration: "40s" }] },
  };
  const seriesTemplates = [
    "6x1000m r:90s (ritmo 4:00–4:05)", "5x1200m r:2min (ritmo 4:00–4:05)", "8x800m r:75s (ritmo 3:55–4:00)",
    "Rodaje suave 8km (sin series - semana recuperación)", "5x1600m r:2min (ritmo 4:10–4:15)", "6x1200m r:90s (ritmo 3:58–4:03)",
    "4x2000m r:2:30min (ritmo 4:10–4:15)", "3x3000m r:3min (ritmo 4:12–4:18)", "10x600m r:60s (ritmo 3:48–3:53)",
    "5x2000m r:2min (ritmo 4:08–4:13)", "3x4000m r:3min (ritmo 4:10–4:14)", "Tempo continuo 6km (ritmo 4:18–4:22)",
    "4x3000m r:2:30min (ritmo 4:07–4:12)", "Rodaje mixto: 5km fácil + 5km ritmo maratón",
    "3x2000m r:2min (ritmo 4:05–4:10) + 6x400m r:60s (ritmo 3:45)", "4x1600m r:2min (ritmo 4:08–4:13)",
  ...
  while (cur <= raceDay) {
    ...
    const weekNum = Math.floor((cur - start) / 86400000 / 7) + 1;
    let phase = "Base 1", longRunKm = 16, isRecoveryWeek = false, isTaperWeek = weekNum === 17;
    ...
    let session = null;
    if (isRaceDay) {
      session = { type: "race", label: "🏆 MEDIA MARATÓN VALLADOLID", detail: "Salida controlada 4:20/km los primeros 5km. Del km 5–10 baja a 4:16/km. Del km 10–18 mantén 4:12–4:15/km. Últimos 3km a fondo.", distance: 21.1, targetTime: "1:28:30–1:30:00" };
    } else if (dow === 1) {
      const easyKm = isRecoveryWeek ? 8 : (weekNum <= 3 ? 10 : weekNum <= 6 ? 11 : weekNum <= 10 ? 12 : weekNum <= 14 ? 13 : isTaperWeek ? 8 : 12);
      session = { type: "easy", label: "Rodaje suave", distance: easyKm, pace: isRecoveryWeek ? pace.recovery : pace.easy, duration: `${Math.round(easyKm * 5.8)}–${Math.round(easyKm * 6.2)} min`, detail: "Ritmo conversacional. FC < 140 ppm." };
    } else if (dow === 2) {
      const gt = weekNum <= 4 ? "base" : weekNum <= 10 ? "mid" : isTaperWeek ? "taper" : "specific";
      session = { type: "gym", label: "Gimnasio", gymType: gt, gymData: gym[gt], detail: "Calentamiento 10 min cardiovascular suave. Estiramientos al final 10 min." };
    } else if (dow === 3) {
      const sIdx = Math.min(seriesIdx, seriesTemplates.length - 1);
      const wc = weekNum <= 6 ? 2 : 3;
      session = { type: "series", label: "Series / Calidad", seriesDescription: seriesTemplates[sIdx], totalKm: weekNum <= 6 ? 10 : 14, detail: `Calentamiento ${wc}km + ${seriesTemplates[sIdx]} + Enfriamiento ${wc}km. Total aprox ${weekNum <= 6 ? 10 : 14}km` };
      seriesIdx++;
    } else if (dow === 4) {
      if (swimAvailable) session = { type: "swim", label: "Natación", detail: "1500–2000m en piscina. Crol. Series 100m con 20s descanso.", distance: 1500 + Math.min((weekNum - 3) * 100, 1000) };
      else session = { type: "rest", label: "Descanso activo", detail: "Movilidad y estiramientos 20 min." };
    } else if (dow === 5) {
      const bk = isRecoveryWeek ? 40 : (weekNum <= 6 ? 50 : weekNum <= 12 ? 65 : isTaperWeek ? 35 : 55);
      session = { type: "bike", label: "Bicicleta", distance: bk, detail: `${bk}km en bici. Zona 2 principalmente (FC 130–150).`, duration: `${Math.round(bk * 2.5)}–${Math.round(bk * 3)} min` };
    } else if (dow === 6) {
      if (weekNum % 2 === 0) { const gt = weekNum <= 4 ? "base" : weekNum <= 10 ? "mid" : isTaperWeek ? "taper" : "specific"; session = { type: "gym", label: "Gimnasio", gymType: gt, gymData: gym[gt], detail: "Segunda sesión de fuerza semanal." }; }
      else { const ek = Math.round(longRunKm * 0.55); session = { type: "easy", label: "Rodaje medio", distance: ek, pace: pace.easy, duration: `${Math.round(ek * 5.7)}–${Math.round(ek * 6.1)} min`, detail: "Rodaje de apoyo, sin forzar." }; }
    } else if (dow === 0) {
      session = { type: "long", label: isTaperWeek ? "Rodaje largo (taper)" : "Rodaje largo", distance: isTaperWeek ? 13 : longRunKm, pace: isRecoveryWeek ? pace.recovery : pace.easy, detail: isRecoveryWeek ? `${longRunKm}km a ${pace.recovery}. Semana recuperación.` : `${longRunKm}km. Primeros 2/3 a ${pace.easy}. Últimos ${Math.round(longRunKm * 0.25)}km a ${pace.marathon}.`, includes: isRecoveryWeek ? null : `Últimos ${Math.round(longRunKm * 0.25)}km a ritmo objetivo (4:15–4:20/km)` };
    }
    if (session) days.push({ date: dateStr, weekNum, phase, dayOfWeek: dow, session, completed: false, actual: null });
...
const UNASSIGNED_KEY = "strava_unassigned";

function loadData() { try { const s = localStorage.getItem(STORAGE_KEY); if (s) return JSON.parse(s); } catch (e) {} return null; }
function saveData(d) { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(d)); } catch (e) {} }
function loadStravaToken() { try { const s = localStorage.getItem(STRAVA_TOKEN_KEY); if (s) return JSON.parse(s); } catch (e) {} return null; }
function saveStravaToken(t) { try { localStorage.setItem(STRAVA_TOKEN_KEY, JSON.stringify(t)); } catch (e) {} }
function loadUnassigned() { try { const s = localStorage.getItem(UNASSIGNED_KEY); if (s) return JSON.parse(s); } catch (e) {} return []; }
function saveUnassigned(u) { try { localStorage.setItem(UNASSIGNED_KEY, JSON.stringify(u)); } catch (e) {} }
...
function formatDate(ds) { const d = new Date(ds + "T00:00:00"); return `${DOW_ES[d.getDay()]} ${d.getDate()} ${MONTH_ES[d.getMonth()]}`; }
...
function actLabel(act) { const c = activityCategory(act); return c === "run" ? "Carrera" : c === "bike" ? "Bici" : c === "swim" ? "Natación" : act.type; }

// ============================================================
// UNASSIGNED TRAY MODAL
// ============================================================
function UnassignedTray({ unassigned, plan, onAssign, onDismiss, onClose }) {
  const [selected, setSelected] = useState(null); // activity index
  const [targetDate, setTargetDate] = useState("");

  const runDays = plan.filter(d => ["easy", "long", "series", "race"].includes(d.session.type));
  const bikeDays = plan.filter(d => d.session.type === "bike");
  const swimDays = plan.filter(d => d.session.type === "swim");

  function compatibleDays(act) {
    const cat = activityCategory(act);
    if (cat === "run") return runDays;
    if (cat === "bike") return bikeDays;
    if (cat === "swim") return swimDays;
    return [];
  }

  const selAct = selected !== null ? unassigned[selected] : null;
  const days = selAct ? compatibleDays(selAct) : [];
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div style={{ background: "#0f172a", border: "1px solid #334155", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 600, maxHeight: "85vh", overflowY: "auto", padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: "#f8fafc" }}>⚠️ Actividades sin asignar</div>
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>Estas actividades no coinciden con ningún día del plan</div>
          </div>
        ...
        {!selAct ? (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#f8fafc", marginBottom: 3 }}>
                        {actIcon(act)} {act.name}
                      </div>
                      <div style={{ fontSize: 11, color: "#64748b", marginBottom: 4 }}>
                        {actLabel(act)} · {actDate ? formatDate(actDate) : "—"}
                      </div>
                      <div style={{ fontSize: 12, color: "#94a3b8" }}>
                        📏 {distKm}km · ⏱ {timeStr}{paceStr ? ` · ${paceStr}/km` : ""}
                      </div>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => { setSelected(i); setTargetDate(""); }} style={{ flex: 1, padding: "8px 0", background: "#6366f122", border: "1px solid #6366f1", borderRadius: 8, color: "#818cf8", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                      📅 Asignar a un día
                    </button>
                    <button onClick={() => onDismiss(act.id)} style={{ padding: "8px 12px", background: "#1e293b", border: "1px solid #334155", borderRadius: 8, color: "#475569", fontSize: 11, cursor: "pointer" }}>
                      🗑 Ignorar
                    </button>
                  </div>
                </div>
          ...
          // Day picker for selected activity
          <div>
            <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", color: "#6366f1", fontSize: 12, cursor: "pointer", marginBottom: 12, padding: 0 }}>
              ← Volver
            </button>
            <div style={{ background: "#1e293b", borderRadius: 10, padding: 12, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 11, color: "#64748b" }}>
                {(selAct.distance / 1000).toFixed(1)}km · {secondsToTime(selAct.moving_time)}
                {activityCategory(selAct) === "run" ? ` · ${speedToPace(selAct.average_speed)}/km` : ""}
              </div>
            </div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 10, letterSpacing: 1 }}>
              ELIGE EL DÍA DEL PLAN AL QUE ASIGNAR:
            </div>
            <div style={{ maxHeight: 320, overflowY: "auto" }}>
              {days.map(d => (
                <button key={d.date} onClick={() => setTargetDate(d.date)} style={{
                  width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center",
                  padding: "10px 12px", marginBottom: 6, borderRadius: 10, cursor: "pointer",
                  background: targetDate === d.date ? "#6366f133" : "#1e293b",
                  border: targetDate === d.date ? "1px solid #6366f1" : "1px solid #334155",
                  textAlign: "left",
                }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#f8fafc" }}>
                      {typeIcon(d.session.type)} {d.session.label}
                    </div>
                    <div style={{ fontSize: 10, color: "#64748b", marginTop: 2 }}>
                      {formatDate(d.date)} · Sem {d.weekNum}
                    </div>
                  </div>
                  <div style={{ fontSize: 11, color: d.completed ? "#4ade80" : "#475569" }}>
                    {d.completed ? "✓ hecho" : "pendiente"}
                  </div>
                </button>
              ))}
            </div>
            {targetDate && (
              <button onClick={() => { onAssign(selAct, targetDate); setSelected(null); }} style={{
                ...
                ✅ Asignar al {formatDate(targetDate)}
              </button>
            )}
          </div>
        )}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, padding: 24, width: "100%", maxWidth: 400, maxHeight: "80vh", overflowY: "auto" }}>
        ...
        {day.actual?.fromStrava && <div style={{ background: "#0c2a1a", border: "1px solid #166534", borderRadius: 8, padding: 10, marginBottom: 12, fontSize: 11, color: "#4ade80" }}>🟠 Importado de Strava: <strong>{day.actual.stravaName}</strong></div>}
        {!isGym && (<>
          <label style={labelStyle}>Distancia real (km)</label>
          <input style={inputStyle} type="number" step="0.1" value={form.distance} onChange={e => setForm(f => ({ ...f, distance: e.target.value }))} />
          <label style={labelStyle}>Tiempo real</label>
          ...
          <label style={labelStyle}>Ritmo medio (/km)</label>
        ...
        <textarea style={{ ...inputStyle, height: 70, resize: "vertical" }} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Sensaciones..." />
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 9, fontWeight: 700, background: color + "22", color, padding: "2px 6px", borderRadius: 6 }}>{session.type.toUpperCase()}</span>
          ...
          {actual && (
            <div style={{ marginTop: 10, padding: "8px 10px", background: "#1e3a5f", borderRadius: 8, fontSize: 11 }}>
              <span style={{ color: actual.fromStrava ? "#f97316" : "#4ade80", fontWeight: 700 }}>{actual.fromStrava ? "🟠 Strava: " : "✓ Realizado: "}</span>
...
function WeekView({ weekDays, onComplete, onUncomplete, onLog }) {
  ...
  const pct = weekDays.length ? Math.round((completedCount / weekDays.length) * 100) : 0;
  return (
    <div style={{ marginBottom: 20 }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 10, padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: open ? 10 : 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: "#818cf8", background: "#818cf822", padding: "2px 8px", borderRadius: 6 }}>{phase.toUpperCase()}</span>
          <span style={{ fontSize: 11, color: "#64748b" }}>~{Math.round(totalKm)}km</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ fontSize: 11, color: completedCount === weekDays.length ? "#4ade80" : "#64748b" }}>{completedCount}/{weekDays.length}</div>
          <div style={{ width: 40, height: 4, background: "#1e293b", borderRadius: 2, overflow: "hidden" }}><div style={{ width: `${pct}%`, height: "100%", background: "#6366f1", borderRadius: 2 }} /></div>
...
function StatsPanel({ plan }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10, marginBottom: 20 }}>
      {[{ label: "Días para la carrera", value: daysLeft, color: "#fbbf24" }, { label: "Sesiones completadas", value: `${done}/${plan.length}`, color: "#4ade80" }, { label: "Km planificados", value: `${Math.round(kmPlanned)}km`, color: "#818cf8" }, { label: "Km reales", value: `${Math.round(kmDone)}km`, color: "#38bdf8" }].map(stat => (
...
export default function App() {
  ...
  useEffect(() => { saveUnassigned(unassigned); }, [unassigned]);
  ...
  async function syncStrava() {
    ...
    try {
      ...
      if (Date.now() / 1000 > token.expires_at - 300) {
        ...
        if (refreshed.access_token) { token = { ...token, ...refreshed }; setStravaToken(token); }
      }
      const activities = await fetchStravaActivities(token.access_token);
      if (Array.isArray(activities)) {
        const { matched, unassigned: newUnassigned } = matchActivitiesToPlan(activities, plan);
        setPlan(matched);
        // Merge unassigned (avoid duplicates)
        setUnassigned(prev => {
          const existingIds = new Set(prev.map(a => a.id));
          const toAdd = newUnassigned.filter(a => !existingIds.has(a.id));
          return [...prev, ...toAdd];
        });
        const ua = newUnassigned.length;
        setSyncMsg(`✅ ${activities.length} actividades procesadas${ua > 0 ? ` · ⚠️ ${ua} sin asignar` : ""}`);
        if (ua > 0) setTimeout(() => setShowTray(true), 1500);
      } else {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020817", color: "#f8fafc", fontFamily: "'Syne', system-ui, sans-serif", paddingBottom: 80 }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #020817; } ::-webkit-scrollbar-thumb { background: #1e293b; border-radius: 2px; } button:hover { opacity: 0.85; }`}</style>
        ...
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 20, fontWeight: 800 }}>Valladolid <span style={{ color: "#6366f1" }}>27/09/26</span></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
              ...
              <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                {stravaToken ? (
                  <>
                    {unassigned.length > 0 && (
                      <button onClick={() => setShowTray(true)} style={{ padding: "5px 8px", background: "#fbbf2422", border: "1px solid #fbbf24", borderRadius: 8, color: "#fbbf24", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>
                        ⚠️ {unassigned.length}
                      </button>
                    )}
                    ...
                    <button onClick={() => { setStravaToken(null); localStorage.removeItem(STRAVA_TOKEN_KEY); }} style={{ padding: "5px 8px", background: "#1e293b", border: "1px solid #334155", borderRadius: 8, color: "#64748b", fontSize: 10, cursor: "pointer" }}>✕</button>
                  </>
                ) : (
                  <a href={stravaAuthUrl()} style={{ padding: "5px 10px", background: "#fc4c0222", border: "1px solid #fc4c02", borderRadius: 8, color: "#fc4c02", fontSize: 10, fontWeight: 700, textDecoration: "none" }}>🟠 Conectar Strava</a>
                )}
              </div>
            </div>
          </div>

          {syncMsg && <div style={{ background: "#1e293b", borderRadius: 8, padding: "8px 12px", fontSize: 11, color: "#94a3b8", marginBottom: 10, textAlign: "center" }}>{syncMsg}</div>}
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "16px" }}>
        ...
        {view === "calendar" && (
          <>
            ...
            {/* Unassigned banner */}
            {unassigned.length > 0 && (
              <button onClick={() => setShowTray(true)} style={{ width: "100%", background: "#fbbf2411", border: "1px solid #fbbf24", borderRadius: 12, padding: "12px 16px", marginBottom: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#fbbf24" }}>⚠️ {unassigned.length} actividad{unassigned.length > 1 ? "es" : ""} de Strava sin asignar</div>
                  <div style={{ fontSize: 11, color: "#92400e", marginTop: 2 }}>Pulsa para asignarlas al día que corresponda</div>
                </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, marginBottom: 8 }}>
                {phases.map(p => <button key={p} onClick={() => setFilterPhase(p)} style={{ whiteSpace: "nowrap", padding: "5px 10px", borderRadius: 8, border: "none", cursor: "pointer", background: filterPhase === p ? "#6366f1" : "#1e293b", color: filterPhase === p ? "#fff" : "#64748b", fontSize: 10, fontWeight: 700 }}>{p === "all" ? "Todas" : p}</button>)}
              </div>
              <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, marginBottom: 8 }}>
                {types.map(t => <button key={t} onClick={() => setFilterType(t)} style={{ whiteSpace: "nowrap", padding: "5px 10px", borderRadius: 8, border: filterType === t ? `1px solid ${typeColor(t)}` : "1px solid transparent", cursor: "pointer", background: filterType === t ? typeColor(t) + "55" : "#1e293b", color: filterType === t ? typeColor(t) : "#64748b", fontSize: 10, fontWeight: 700 }}>{typeIcon(t)} {t === "all" ? "Todo" : t}</button>)}
              </div>
        ...
        {view === "stats" && (
          <div>
              ...
              {["easy", "long", "series", "gym", "bike", "swim"].map(type => {
                ...
                return (
                  <div key={type} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ fontSize: 11, color: "#94a3b8" }}>{typeIcon(type)} {type}</span><span style={{ fontSize: 11, color: typeColor(type) }}>{done}/{total}</span></div>
                    <div style={{ height: 4, background: "#1e293b", borderRadius: 2 }}><div style={{ height: "100%", width: `${Math.round((done / total) * 100)}%`, background: typeColor(type), borderRadius: 2 }} /></div>
                  </div>
                );
              })}
            </div>
            <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 16, marginBottom: 12 }}>
              ...
              {stravaToken ? (
                <div>
                  ...
                  {unassigned.length > 0 && <div style={{ fontSize: 11, color: "#fbbf24", marginBottom: 8 }}>⚠️ {unassigned.length} actividad{unassigned.length > 1 ? "es" : ""} pendiente{unassigned.length > 1 ? "s" : ""} de asignar</div>}
                ...
                <div>
                  <div style={{ fontSize: 11, color: "#64748b", marginBottom: 12 }}>Conecta Strava para importar automáticamente tus entrenos.</div>
            ...
            <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 16 }}>
              ...
              {[{ label: "HM Madrid 26 Abr", value: "1:31:48", sub: "4:21/km" }, { label: "10k Familias", value: "40:04", sub: "4:00/km" }, { label: "Objetivo Valladolid", value: "< 1:30:00", sub: "4:15/km", highlight: true }, { label: "Ritmo series", value: "3:55–4:05/km", sub: "VO2max" }, { label: "Ritmo umbral", value: "4:15–4:25/km", sub: "Tempo" }, { label: "Rodaje fácil", value: "5:40–6:10/km", sub: "Z2" }].map(r => (
                <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #1e293b" }}>
                  ...
                  <div style={{ textAlign: "right" }}><div style={{ fontSize: 13, fontWeight: 700, color: r.highlight ? "#fbbf24" : "#f8fafc" }}>{r.value}</div><div style={{ fontSize: 10, color: "#475569" }}>{r.sub}</div></div>
                </div>
              ))}
            </div>
          </div>
        ...
        {view === "race" && (
          <div>
            <div style={{ background: "linear-gradient(135deg, #1c1917 0%, #0f172a 100%)", border: "1px solid #fbbf2433", borderRadius: 16, padding: 20, marginBottom: 16 }}>
              ...
              {[{ km: "0–5", pace: "4:20/km", time: "~21:40", note: "Salida controlada, no te dejes llevar" }, { km: "5–10", pace: "4:16/km", time: "~21:20", note: "Ritmo objetivo, encuentra el surco" }, { km: "10–18", pace: "4:12/km", time: "~33:36", note: "Bloque principal, mantén" }, { km: "18–21.1", pace: "4:05/km", time: "~12:42", note: "Si te quedan fuerzas, ¡aprieta!" }].map(s => (
              ...
              <div style={{ background: "#1e293b", borderRadius: 10, padding: 12, marginTop: 16, marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#38bdf8", marginBottom: 6 }}>🍌 DÍA ANTES</div>
                <div style={{ fontSize: 11, color: "#94a3b8", lineHeight: 1.7 }}>· Pasta o arroz, nada nuevo<br/>· Hidratación extra<br/>· Paseo 15–20min<br/>· En cama a las 22:30</div>
              </div>
            ...
            <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 16 }}>
              ...
              <div style={{ fontSize: 11, color: "#64748b", lineHeight: 1.8 }}>Con tus tiempos actuales (HM 1:31:48, 10k 40:04) tu <strong style={{ color: "#818cf8" }}>ritmo umbral</strong> está ~4:18–4:22/km.<br/><br/>Para bajar de 1:30 necesitas ~4:15/km. En 17 semanas:<br/>· Km semanales: 45 → 68 → taper<br/>· Series progresivas para bajar VMA<br/>· Fuerza para economía de carrera<br/><br/>Mejora esperada: <strong style={{ color: "#4ade80" }}>1:28:30–1:29:30</strong> 🎯</div>
            </div>
          </div>
        )}
      </div>
